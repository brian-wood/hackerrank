...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    for (i = 0; i < player_count; i++) {
        for(j = 0, rank = 1; j < ranked_count; j++, rank++) {
            fprintf(stderr, "player[%d]: %d, ranked[%d]: %d, rank: %d\n",
                    i, player[i], j, ranked[rank], rank);
            if (player[i] >= ranked[j]) { // if player score better than curr top score, new #1
                n_rank[i] = rank;
                break;
            } else if (ranked[j] == ranked[j+1]) { // skip the same ranked player scores
                while (ranked[j] == ranked[j+1]) {
                    j++;
                    fprintf(stderr, "  skipped ranked[%d]: %d, rank: %d\n", j, ranked[rank], rank);
                }
            }
        ...
        if (n_rank[i] == 0)
            n_rank[i] = rank;
        fprintf(stderr, "  n_ranked[%d]: %d, rank: %d\n", i, n_rank[i], rank);
    }

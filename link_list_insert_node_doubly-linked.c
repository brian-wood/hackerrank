...
DoublyLinkedListNode* sortedInsert(DoublyLinkedListNode* llist, int data) {
    ...
    struct DoublyLinkedListNode *new_node = calloc(1, sizeof(struct DoublyLinkedListNode));
    struct DoublyLinkedListNode *cur_node = NULL, *tmp_prev = NULL;
    
    new_node->data = data;
    new_node->prev = NULL;
    new_node->next = NULL;

    // quick sanity check...if llist == NULL (empty list), set and return list
    if (llist == NULL) {
        llist = new_node;
        return llist;
    }

    // find value of node that is larger than our "data" value passed in
    cur_node = llist;
    while(cur_node != NULL) {
    ...
    // cur_node will point to node data value > input "data" value OR it will be end of list
    new_node->next = cur_node;
    if (tmp_prev == NULL) { // inserting at head of list
        llist = new_node;
    } else {

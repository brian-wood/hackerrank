 ...
 *
 */
/* Recursive search function to find score to ranking
 * parameters: player - player score to compare
 *             scores - ranked player scores array
 *             index  - ranked player array index
 *             max_index - maximum recursive calls.
 * return:     score ranking
 */
int get_rank(int player, int* scores, int index, int rank, int max_index)
{
    if (player >= scores[index])
        return rank;
        ...
        rank++;
        return get_rank(player, scores, ++index, rank, max_index);
    }
}
...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    for (int i = 0; i < player_count; i++) {
        n_rank[i] = get_rank(player[i], ranked, 0, 1, ranked_count); // add 1 to zero based index
    }

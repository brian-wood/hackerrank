...
void checkMagazine(int magazine_count, char** magazine, int note_count, char** note) {
    ...
    int note_match = 0; // record matched words, if equal to note then we have 
    // check each word in note
    for (int i = 0; i < note_count; i++) {
        // compare each notes word (case dependant) to magazine words
        for (int j = 0; j < magazine_count; j++) {
            if (magazine[j][0] == note[i][0]) { // check if first character matches, skip full compare if not
                if (!(strncmp(&magazine[j][0], &note[i][0], strlen(&note[i][0])))) {
                    note_match++;
                    strcpy(&magazine[j][0]," ");
                    break;
                }

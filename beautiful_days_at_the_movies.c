...
int beautifulDays(int i, int j, int k)
{
    ...
    int btfl_days = 0, reverse_i = 0;
    int day = i;

    for (int x = 0, y = day; x <= (j-i);
        x++, ++day, y = day, reverse_i = 0) {
        // abs(i-reverse(i)) % k
        while(y != 0) {
            reverse_i = reverse_i * 10;
            reverse_i = reverse_i + (y % 10);
            y = y / 10;
        }

        if (!(abs(day - reverse_i) % k))
            btfl_days++;

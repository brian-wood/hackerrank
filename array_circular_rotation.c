...
int* circularArrayRotation(int a_count, int* a, int k, int queries_count, int* queries, int* result_count) {
    ...
    int arr_overage;
    int *ret_queries = (int*)malloc(queries_count * sizeof(int));
    *result_count = queries_count;
    memset(ret_queries, 0, queries_count);
    
    for (int i = 0; i < queries_count; i++) {
        if (queries[i] >= k) {
            ret_queries[i] = a[queries[i] - k];
        } else if (k > a_count){
            arr_overage = queries[i] + a_count - (k % a_count);
            if (arr_overage >= a_count)
                arr_overage -= a_count;
            ret_queries[i] = a[arr_overage];
        } else {
            ret_queries[i] = a[queries[i]+a_count-k];
        }   
    }
